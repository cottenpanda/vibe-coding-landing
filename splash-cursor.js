// SplashCursor - Vanilla JS port of reactbits SplashCursor
// WebGL fluid simulation that creates liquid splash effects following cursor movement
// Original: github.com/DavidHDev/react-bits
...
function initSplashCursor(canvas) {
  ...
  let config = {
    ...
    COLOR_UPDATE_SPEED, PAUSED: false, BACK_COLOR: { r: 0, g: 0, b: 0 }, TRANSPARENT
  };
  ...
  function drawDisplay(target){let w=target==null?gl.drawingBufferWidth:target.width;let h=target==null?gl.drawingBufferHeight:target.height;displayMaterial.bind();if(config.SHADING)gl.uniform2f(displayMaterial.uniforms.texelSize,1/w,1/h);gl.uniform1i(displayMaterial.uniforms.uTexture,dye.read.attach(0));blit(target);}
  ...
  function getResolution(resolution){let ar=gl.drawingBufferWidth/gl.drawingBufferHeight;if(ar<1)ar=1/ar;const min=Math.round(resolution);const max=Math.round(resolution*ar);if(gl.drawingBufferWidth>gl.drawingBufferHeight)return{width:max,height:min};return{width:min,height:max};}
  ...
  function handleMouseDown(e){let pointer=pointers[0];updatePointerDownData(pointer,-1,scaleByPixelRatio(e.clientX),scaleByPixelRatio(e.clientY));clickSplat(pointer);}
  let firstMove=false;
  function handleMouseMove(e){let pointer=pointers[0];let posX=scaleByPixelRatio(e.clientX);let posY=scaleByPixelRatio(e.clientY);if(!firstMove){updatePointerMoveData(pointer,posX,posY,generateColor());firstMove=true;}else{updatePointerMoveData(pointer,posX,posY,pointer.color);}}
  ...
  function handleTouchEnd(e){let pointer=pointers[0];updatePointerUpData(pointer);}
